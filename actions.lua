...
local function onNext(isbuyer)
    ...
    local function redeemCodes()
        ...
        for _, code in pairs(codes:GetChildren()) do
            if code:IsA("IntValue") then
                ...
                if fallo then
                    warn("Error in redeem code: "..fallo)
                else
                    warn("Redeem code success!")
                end
            end
    ...
    local function executeReb()
        if autoFarmValues.autoRebirth and game.PlaceId ~= earthId then
            events:WaitForChild("reb"):InvokeServer()
        end
    end
                ...
                local x, y = pcall(function()
                    if melee.stats <= myStats then
                        ...
                        if game.PlaceId == earthId then
                            warn("Earh planet attacks")
                            events:FindFirstChild("Haha"):InvokeServer(unpack(args))
                        else
                            warn("Other planets attacks")
                            events:FindFirstChild('letsplayagame'):InvokeServer(unpack(args))
                        end
